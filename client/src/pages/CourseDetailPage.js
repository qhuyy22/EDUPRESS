...
const CourseDetailPage = () => {
  ...
  const fetchCourse = async () => {
    try {
      ...
      setError(err.response?.data?.message || 'Tải chi tiết khóa học thất bại');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleEnroll = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Đăng ký khóa học thất bại');
    } finally {
      setEnrolling(false);
    }
  };
  ...
  if (loading) {
    return <Loader message="Đang tải chi tiết khóa học..." />;
  }
              ...
              {course.averageRating > 0 && (
                <div className="meta-item">
                  ...
                  <span className="meta-value">
                    ⭐ {course.averageRating.toFixed(1)} ({course.totalReviews} đánh giá)
                  </span>
                </div>
              )}
              <div className="meta-item">
          ...
          <div className="course-details">
            ...
            {course.lessons && course.lessons.length > 0 && (
              <div className="course-lessons">
                <h3>Nội dung Khóa học ({course.lessons.length} bài học)</h3>
